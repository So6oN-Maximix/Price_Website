async function addToComment(postInfo, containerId) {
    ...
    commentButton.addEventListener("click", async () => {
        ...
        document.getElementById("new-comment-form").dataset.postId = postInfo.inspi_comment_id;
...
async function loadPostComments(postId) {
    ...
    if (commentListFetch.ok) {
        ...
        if (commentList.length === 0) {
            commentListContainer.innerHTML = '<p style="color: rgba(255,255,255,0.5); text-align: center; font-size: 0.9rem; margin: auto;">Soyez le premier à commenter !!</p>';
            return;
        }
