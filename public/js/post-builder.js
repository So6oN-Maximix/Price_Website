async function addToComment(postInfo, containerId) {
  ...
  const userInfosQuery = await fetch(
    `/api/get-user-info?id=${postInfo.user_id}`,
  );
  ...
  if (currentUserId === postInfo.user_id) {
    ...
    deleteButton.addEventListener("click", async () => {
      if (
        confirm(
          "Es-tu sûr de vouloir supprimer ce magnifique setup ? L'action est irréversible !",
        )
      ) {
        try {
...
async function addComment(commentObj, containerId) {
  ...
  let userUsername = "";
  const userInfosQuery = await fetch(
    `/api/get-user-info?id=${commentObj.user_id}`,
  );
  if (userInfosQuery.ok) {
    const userInfos = await userInfosQuery.json();
    userImg.src = userInfos.profil_pic;
  ...
  if (currentUserId === commentObj.user_id) {
    ...
    suppButton.addEventListener("click", async () => {
      if (
        confirm(
          "Es-tu sûr de vouloir supprimer ce commentaire ? L'action est irréversible !",
        )
      ) {
        try {
          ...
          if (response.ok) {
            commentItem.remove();
            const nbCommentsSpan = document.getElementById(
              `post-${commentObj.post_id}`,
            );
            if (nbCommentsSpan) {
              nbCommentsSpan.textContent =
                Number(nbCommentsSpan.textContent) - 1;
            }
          } else {
            alert("Erreur lors de la suppression.");
          }
  ...
  likeButton.innerHTML = `
    ...
    `;
  const isLikedFetch = await fetch(
    `/api/check-comment-like?id=${commentObj.post_comment_id}`,
  );
  if (isLikedFetch.ok) {
    const isLiked = await isLikedFetch.json();
    if (isLiked) {
